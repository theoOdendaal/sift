...
fn run_w3c_xml_conformance_test_suites() {
    ...
    if !manifest_path.exists() {
        panic!(
            "w3c XML Conformance Test Suites not found, please download from: https://www.w3.org/XML/Test/"
        )
    }

    let manifest_content = std::fs::read_to_string(manifest_path)
        .expect("Unable to read w3c XML Conformance Test Suites manifest");
